...
def preprocess_location(name):
    ...
    """
    # discard = {
    #     'hospital', 'bed', 'govt', 'government', 'clinic', 'upazila', 'district', 'general', 'sadar',
    #     'medical', 'college', 'rhc', 'uhc', 'dh', 'and', 'of', 'the', 'in', '&', 'chest', 'disease',
    #     'trauma', 'center', 'specialized', 'leprosy', 'infectious', 'tb', 'eye', 'burn', 'maternal',
    #     'child', 'health', 'training', 'institute', 'national', 'bangladesh', 'principal', 'director',
    #     '10', '20', '25', '30', '31', '50', '250', '300', '500'
    # }
    discard = {
        "hospital",
        "bed",
...
#             model="whisper-1", file=("audio_input.wav", audio_bytes), language="en"
#         )
...
#     """
#     Transcribe audio from bytes (e.g., received from a Streamlit microphone).
#     Resamples to 16 kHz mono WAV to reduce file size, saves a copy locally, then uses Whisper for English (en) transcription.
#     """
#     if not audio_bytes:
#         logger.warning("Empty audio bytes provided to transcribe_with_openai")
#         return ""
...
#     try:
#         # Load audio bytes into AudioSegment (assuming input is WAV)
#         audio_segment = AudioSegment.from_wav(io.BytesIO(audio_bytes))
...
#         processed_audio_bytes = wav_buffer.getvalue()

#         # Generate unique filename with timestamp
#         timestamp_str = datetime.now().strftime("%Y%m%d_%H%M%S")
#         wav_filename = f"{audio_dir}/recording_{timestamp_str}_16khz_mono.wav"

#         # Save processed WAV copy locally
#         with open(wav_filename, "wb") as f:
#             f.write(processed_audio_bytes)
#         logger.info(f"Processed 16 kHz mono WAV saved locally: {wav_filename}")

#         # Transcribe the processed WAV with OpenAI Whisper
#         response = client.audio.transcriptions.create(
#             model="whisper-1", file=("audio_input_16khz_mono.wav", processed_audio_bytes), language="en"
#         )
#         logger.info("Audio transcription completed successfully")
#         return response.text
#     except Exception as e:
...
# -----------------------------
# Transcription (Speech-to-Text - STT)
# WAV to WebM/Opus (64 kbps)
# -----------------------------
def transcribe_with_openai(audio_bytes):
    """
    Transcribe audio from bytes (e.g., received from a Streamlit microphone).
    Converts to WebM/Opus (64 kbps) to reduce file size, saves a copy locally, then uses Whisper for English (en) transcription.
    """
    if not audio_bytes:
        logger.warning("Empty audio bytes provided to transcribe_with_openai")
        return ""
    ...
    try:
        ...
        # Optional: Resample to 16 kHz and mono for extra efficiency (Opus works well here)
        audio_segment = audio_segment.set_frame_rate(16000).set_channels(1)

        # Export to WebM/Opus bytes (64 kbps for voice-optimized compression)
        webm_buffer = io.BytesIO()
        audio_segment.export(webm_buffer, format="webm", codec="libopus", bitrate="64k")
        webm_bytes = webm_buffer.getvalue()

        # Generate unique filename with timestamp
        timestamp_str = datetime.now().strftime("%Y%m%d_%H%M%S")
        webm_filename = f"{audio_dir}/recording_{timestamp_str}_webm_opus.webm"

        # Save WebM copy locally
        with open(webm_filename, "wb") as f:
            f.write(webm_bytes)
        logger.info(f"WebM/Opus audio saved locally: {webm_filename} (reduced size)")

        # Transcribe the WebM with OpenAI Whisper
        response = client.audio.transcriptions.create(
            model="whisper-1", file=("audio_input.webm", webm_bytes), language="en"
        )
        logger.info("Audio transcription completed successfully")
        return response.text
    except Exception as e:
